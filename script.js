...
function startGame() {
    ...
    div.x = 663;  //  Used to move the starting
    div.y = 500;  //  position of car.
...
function moveBadGuys() {
    ...
    for (let i = 0; i < tempBaddy.length; i++) {
        ...
        if (y > 2000 || y < -2000) {
            ...
            if(y>2000){         // If we pass a car then increase our score.
              player.score++;
...
function gameOverPlay(){
 let div = document.createElement('div');
 div.setAttribute('class','final_msg');
 div.style.top = '20px';
...
function playGame() {
    if (gamePlay) {
        ...
        // PLAYER.ELE.X IS THE X POSITION OF CAR AND SAME WITH Y.

        // check if on road if not then decrease the speed.
